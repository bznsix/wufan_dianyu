...
class MyStrategy():
    ...
    def next(self, trend):
        ...
        if long_pos > 0:
            ...
            if self.now_gap < self.params.gap_time and close < self.last_buy_price * (
                    1 - self.params.long_win_ratio) and timestamp_ms > self.last_can_buy:
                size = self.start_cash / self.params.gap_time / close
                print(
                    f'触发加仓，size:{size},当前时间:{timestamp_ms},上次购买{self.last_buy_price},当前是{self.now_gap}次')
                we.send_to_wecom(
                    f'触发加仓，size:{size},当前时间:{timestamp_ms},上次购买{self.last_buy_price},当前是{self.now_gap}次')
                order = self.broker.future.creat_order(self.symbol, 'market', size, 1, 'buy', 'LONG')
                self.long_order[order['orderId']] = order
        ...
        for key, order in self.long_stop_order.items():
            try:
                ...
                if r == 'FILLED':
                    ...
                    we.send_to_wecom(f'第{key}次加仓成功止盈')
        ...
        if len(keys_to_remove) != 0:
            ...
            self.start_cash = self.broker.get_cash() / (
                    self.params.gap_time - self.now_gap) * self.params.gap_time * self.params.margin
            print(f'触发更新起始总资金，起始总资金为->{self.start_cash}')
            we.send_to_wecom(f'触发更新起始总资金，起始总资金为->{self.start_cash}')
